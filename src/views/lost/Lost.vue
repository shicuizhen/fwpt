...
export default {
  ...
  methods: {
    ...
    // 0-10  10-20   20-30
    getPageNum () {
      if (this.lostInformation.length / 10 === 0) {
        this.pageNum = this.lostInformation.length / 10
      } else {
        this.pageNum = Math.ceil(this.lostInformation.length / 10)
      }
    },
    getPage (index) {
      ...
      if (index >= 0 && index < this.pageNum) {
        ...
        console.log('index:')
        console.log(index)
        this.lostInformationPage = this.lostInformation.slice(index * 10, index * 10 + 10)
      }
    },
    // 初始化weosocket
    initWebSocket () {
    ...
    search () {
      ...
      axios({
        ...
        if (resp.data.code === 200) {
          ...
          this.lostInformationPage = this.lostInformation.slice(0, 10)
          this.index = 0
          this.getPageNum()
        }
        this.searchBy = {
    ...
    loadLostInformation () {
      ...
      axios({
        ...
        if (resp.data.code === 200) {
          ...
          _this.lostInformationPage = _this.lostInformation.slice(0, 10)
          _this.index = 0
          _this.getPageNum()
        }
      }).catch(error => error)
